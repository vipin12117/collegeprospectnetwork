...
if($_REQUEST['mode']=="del")
{
	...
	$delete_query_details = "delete from ".TBL_SUBSRIPTION." where fldId=".$fldId;
	$delmsg = $db->query($delete_query_details);
	if(isset($delmsg)){
		$_REQUEST['msg']= "Subsription Deleted Successfully!";
		$count=$func->totalRows(TBL_SUBSRIPTION);
...
if($_REQUEST['mode']=="delAll"){
	...
	for($i=0;$i<count($_POST['check_delete']);$i++)
	{
	 ...
	 $delete_query_details = "delete from ".TBL_SUBSRIPTION." where fldId=".$_POST['check_delete'][$i];
		$delmsg = $db->query($delete_query_details);
	}
	if(isset($delmsg)){
		$_REQUEST['msg']= "Subsription Deleted Successfully!";
...
function deleteRecord(fldId,page) {
	...
	if( confirm("Are you sure to delete the Subsription?")) {
...
function submitDeleteAll(){
	...
	if(countChecked!=0){
		if( confirm("Are you sure to delete the selected Subsription?")) {
		...
		alert("Please select at least one Subsription to delete");
		return false;
	}
			...
			<TD bgColor=#ffffff>
				...
				<!--Main Center Content BEGIN -->
						...
						<table border="1" cellpadding="1" cellspacing="0" width="100%" bordercolor="#808080" style="border-collapse:collapse" class="tablePadd"> 
							...
							<strong>Subsription Listing</strong> </td></tr>
				...
				<input type="hidden" name="page" id="page" value="0">
						...
						<?
						if(!$searchname){
						$query =" Select * from ".TBL_SUBSRIPTION; }else{
						$query ="Select * from ".TBL_SUBSRIPTION." where 1=1 ".$srchCond;
						}
						...
						echo '<tr><td align="center" class="normalblack_12" width="4%">&nbsp;</td>
									...
									<td align="left" class="normalblack_12" width="25%">&nbsp;<strong>Subsription Name</strong></td>
<td align="left" class="normalblack_12" width="25%">&nbsp;<strong>Subsription Cost</strong></td>                                   ';
						...
						for ($i=0;$i<$db->num_rows();$i++) {
							...
							$fldCost			= $func->output_fun($db->f('fldCost'));							
							$pageURL            = "ADSubscribeDetails.php?fldId=$fldId";
							$detailsWindowTitle = "ADSubscribeDetails";
							...
							echo '<tr><td align="center" class="normalblack_12">&nbsp;';
									...
									<td align="left" class="normalblack_12" >'.wordwrap($fldCost
,17,"\n",true).'</td>
									';

																   		
							   		echo '<td class="normalblack_12" align="center"><a href="ADSubsriptionEdit.php?mode=edit&fldId='.$db->f('fldId').'&page='.$pageno.'"><img src="images/b_edit.png" border="0" title="Edit"></a></td>';
