...
{
	$fldPageId=$_REQUEST['fldPageId'];
	$delete_query_details = "delete from ".TBL_PAGE." where fldPageId='".$fldPageId."'";
	$delmsg = $db->query($delete_query_details);
	if(isset($delmsg)){
		$_REQUEST['msg']= "Page Deleted Successfully!";
		$count=$func->totalRows(TBL_PAGE);
...
if($_REQUEST['mode']=="delAll"){
	...
	$delete_query_details = "delete from ".TBL_PAGE." where fldPageId='".$_POST['check_delete'][$i]."'";
		$delmsg = $db->query($delete_query_details);
...
function deleteRecord(fldPageId,page) {
	
	if( confirm("Are you sure to delete the Page?")) {
		document.frmPage.action="?mode=del&fldPageId="+fldPageId+"&page="+page;
...
function changeStatus(toStatus,id,page){
	...
	document.frmPage.action="?mode=changeStatus&st="+toStatus+"&fldPageId="+fldPageId+"&page="+page;
	document.frmPage.submit();
}
...
function submitDeleteAll(){
	...
	if(countChecked!=0){
		if( confirm("Are you sure to delete the selected Page?")) {
			...
			<TD bgColor=#ffffff>
				...
				<!--Main Center Content BEGIN -->
						...
						<table border="1" cellpadding="1" cellspacing="0" width="100%" bordercolor="#808080" style="border-collapse:collapse" class="tablePadd"> 
							...
							<strong>Page Listing</strong> </td></tr>
				...
				<input type="hidden" name="page" id="page" value="0">
							...
							<input type="submit"  name="search" id="search" style="width:180px" value="Search Page By Page title">
							</form></td></tr>
							<form name="frmPage" action="" method="post" onsubmit="">
						<?
						if(!$searchname){
						$query =" Select * from ".TBL_PAGE; }else{
						$query ="Select * from ".TBL_PAGE." where 1=1 ".$srchCond;
						}
