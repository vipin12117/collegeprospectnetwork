...
if($_GET['mode']=='edit' AND $fldId!=""){
	#get the records
	$query =" Select * from ".TBL_HS_AAU_TEAM_NAME. " where fldId =".$fldId;
	...
	if($db->num_rows()>0){
		...
		$fldContactInfo        = $db->f('fldContactInfo');
...
if($_POST['isSubmit']=='save'){
	...
	$where = "fldId=".$fldId;
        			//Update data
				     $strDataArr=array(
		     		     'fldAddress' 			    => $func->input_fun($_POST['fldAddress']),'fldContactInfo' 			    => $func->input_fun($_POST['fldContactInfo']),		
            'fldStatus' 				=> $func->input_fun($_POST['fldStatus']));

			$db->updateRec(TBL_HS_AAU_TEAM_NAME,$strDataArr, $where);
			
			#redirect to listing page on successfull updation
			header("Location: HsAauTeamList.php?page=".$_REQUEST['page']."&msg=HS / AAU  Team Name Updated Successfully!");
...
<HTML><HEAD><TITLE>HS / AAU  Team Name Info</TITLE>
...
<!-- MAin Content Starts From Here -->
			...
			<td valign="top" class="normalwhite_14" colspan=3 bgcolor="#808080" align="center">
				<b>Edit HS / AAU  Team Name Information</b>
			</td>
			</tr>
			<tr>
			<!-- Display the message on heading of the page -->
			...
			<td valign="top"  align="center" class="normalblack_12" > : </td>
			<td valign="top" align="left" class="normalblack_12"><textarea name="fldContactInfo" id="fldContactInfo" rows="10" cols="15"  style="width: 220px;"  class="txt"><?php echo $fldContactInfo; ?></textarea>
            </td>
			</tr>
			<tr height="20">
			<td align="center" valign="top" colspan="3" class="normalwhite_14">
