...
if($_REQUEST['mode']=="del")
{
	...
	$delete_query_details = "delete from ".$tbl_coupon." where id=".$fldEventId;
	$delmsg = $db->query($delete_query_details);
	if(isset($delmsg)){
		$_REQUEST['msg']= "Coupon Deleted Successfully!";
		$count=$func->totalRows($tbl_coupon);
...
if($_REQUEST['mode']=="delAll"){
	...
	for($i=0;$i<count($_POST['check_delete']);$i++)
	{
	 ...
	 $delete_query_details = "delete from ".$tbl_coupon." where id=".$_POST['check_delete'][$i];
		$delmsg = $db->query($delete_query_details);
	}
	if(isset($delmsg)){
		$_REQUEST['msg']= "Coupon Deleted Successfully!";
...
?>
<HTML><HEAD><TITLE>Coupon Listing</TITLE>
...
function deleteRecord(fldEventId,page) {
	...
	if( confirm("Are you sure to delete the Coupon?")) {
...
function submitDeleteAll(){
	...
	if(countChecked!=0){
		if( confirm("Are you sure to delete the selected Coupon?")) {
			...
			<TD bgColor=#ffffff>
				...
				<!--Main Center Content BEGIN -->
						...
						<table border="1" cellpadding="1" cellspacing="0" width="100%" bordercolor="#808080" style="border-collapse:collapse" class="tablePadd"> 
							...
							<strong>Coupon Listing</strong> </td></tr>
				...
				<input type="hidden" name="page" id="page" value="0">
						...
						{
						$query ="Select * from ".$tbl_coupon." where event_id='".$event_id."'";
						}
						else
						{
						...
						echo '<tr><td align="center" class="normalblack_12" width="4%">&nbsp;</td>
									...
									<td align="left" class="normalblack_12" width="25%">&nbsp;<strong>Coupon Number</strong></td>
									<td align="left" class="normalblack_12" width="25%">&nbsp;<strong>Discount</strong></td>
                                   ';
						...
						for ($i=0;$i<$db->num_rows();$i++) {
							...
							$Couponid               = $func->output_fun($db->f('id'));
							$Coupon_number           = $func->output_fun($db->f('cpn_number'));
							$discount_amount           = $func->output_fun($db->f('amount'));							
							$pageURL            = "ADCoupon.php?id=$Couponid";
							$detailsWindowTitle = "Admin Coupon";


							echo '<tr><td align="center" class="normalblack_12">&nbsp;';
									
									echo '<input type="checkbox" id="check_delete[]" name="check_delete[]" value="'.$Couponid.'" onclick="return checkItSelf();">';
									
									echo '<td align="left" class="normalblack_12">&nbsp;'.($count+($_REQUEST['page']*10)).'</td>';
									echo '
									
									
									<td align="left" class="normalblack_12" >'.wordwrap($Coupon_number,17,"\n",true).'</td>
									<td align="left" class="normalblack_12" >'.wordwrap($discount_amount,17,"\n",true).'</td>
									';
							   		
							   		echo '<td class="normalblack_12" align="center"><a href="ADCouponEdit.php?mode=edit&id='.$Couponid.'&page='.$pageno.'"><img src="images/b_edit.png" border="0" title="Edit"></a></td>';
									
							   		echo ' <td  class="normalblack_12" align="center"><a href="javascript:deleteRecord(\''.$Couponid.'\','.$pageno.')"><img src="images/b_drop.png" border="0" title="Delete"></a></td>';
