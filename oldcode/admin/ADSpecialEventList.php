...
if($_REQUEST['mode']=="del")
{
	...
	$delete_query_details = "delete from ".TBL_SPECIAL_EVENT." where fldEventId=".$fldEventId;
	$delmsg = $db->query($delete_query_details);
	if(isset($delmsg)){
		$_REQUEST['msg']= "Special Event Deleted Successfully!";
		$count=$func->totalRows(TBL_SPECIAL_EVENT);
...
if($_REQUEST['mode']=="delAll"){
	...
	for($i=0;$i<count($_POST['check_delete']);$i++)
	{
	 ...
	 $delete_query_details = "delete from ".TBL_SPECIAL_EVENT." where fldEventId=".$_POST['check_delete'][$i];
		$delmsg = $db->query($delete_query_details);
	}
	if(isset($delmsg)){
		$_REQUEST['msg']= "Special Event Deleted Successfully!";
...
?>
<HTML><HEAD><TITLE>Special Event Listing</TITLE>
...
function deleteRecord(fldEventId,page) {
	...
	if( confirm("Are you sure to delete the Special Event?")) {
...
function submitDeleteAll(){
	...
	if(countChecked!=0){
		if( confirm("Are you sure to delete the selected Special Event?")) {
		...
		alert("Please select at least one Special Event to delete");
		return false;
	}
			...
			<TD bgColor=#ffffff>
				...
				<!--Main Center Content BEGIN -->
						...
						<table border="1" cellpadding="1" cellspacing="0" width="100%" bordercolor="#808080" style="border-collapse:collapse" class="tablePadd"> 
							...
							<strong>Special Event Listing</strong> </td></tr>
				...
				<input type="hidden" name="page" id="page" value="0">
						...
						<?
						if(!$searchname){
						$query =" Select * from ".TBL_SPECIAL_EVENT; }else{
						$query ="Select * from ".TBL_SPECIAL_EVENT." where 1=1 ".$srchCond;
						}
						...
						echo '<tr><td align="center" class="normalblack_12" width="4%">&nbsp;</td>
							   	<td align="left" class="normalblack_12" width="8%">&nbsp;<strong>S.No.</strong></td>					<td align="left" class="normalblack_12" width="25%">&nbsp;<strong>Event Name</strong></td>';			echo '<td class="normalblack_12" width="5%" align="center"><strong>Coupon</strong></td>';				echo '<td class="normalblack_12" width="5%" align="center"><strong>Transportation</strong></td>';				echo '<td class="normalblack_12" width="5%" align="center"><strong>Edit</strong></td>';				echo '<td class="normalblack_12" width="7%" align="center"><strong>Delete</strong></td>';		   		echo '<td class="normalblack_12" width="7%" align="center"><strong>Details</strong></td>';
						...
						for ($i=0;$i<$db->num_rows();$i++) {
							...
							$pageURL            = "ADSpecialEventDetails.php?fldEventId=$fldEventId";
							...
							echo '<tr><td align="center" class="normalblack_12">&nbsp;';
									...
									echo '<td align="left" class="normalblack_12" >'.wordwrap($fldEventName,17,"\n",true).'</td>';
									echo '<td class="normalblack_12" align="center"><a href="ADCouponManage.php?fldEventId='.$db->f('fldEventId').'&page='.$pageno.'">View</a></td>';
									echo '<td class="normalblack_12" align="center"><a href="ADTransportation.php?fldEventId='.$db->f('fldEventId').'&page='.$pageno.'">View</a></td>';
							   		echo '<td class="normalblack_12" align="center"><a href="ADSpecialEventEdit.php?mode=edit&fldEventId='.$db->f('fldEventId').'&page='.$pageno.'"><img src="images/b_edit.png" border="0" title="Edit"></a></td>';
