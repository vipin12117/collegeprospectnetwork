...
if($_POST['isSubmit']=='save'){
		...
		if($db->MatchingRec(TBL_BANNER,$whereClause)>0) {    #user Username already exists
			$error_msg = 'This Banner Already Exists!';
			$flag++;
			}
...
$image_name="";
	 	...
	 	$db->insertRec(TBL_BANNER,$strDataArr);
       
			#redirect to listing page on successfull updation
			
			header("Location: ADBannerList.php?msg=Banner is Added Successfully, ");
		}
...
function validate(){
	...
	if((trimString(document.frmBanner.fldThirdParty.value) == "") )
	{
		error_msg += "Enter image   code for display banner! \n";
	}
	if((trimString(document.frmBanner.fldPosition.value) == "") )
	{
		error_msg += "Please Select Banner Position! \n";
	}
	if(error_msg!=''){
		alert(error_msg);
		return false;
