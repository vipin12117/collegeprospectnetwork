...
if($_REQUEST['mode']=="del")
{
	...
	$delete_query_details = "delete from ".TBL_HS_AAU_TEAM_NAME." where fldId=".$fldId;
	$delmsg = $db->query($delete_query_details);
	if(isset($delmsg)){
		$_REQUEST['msg']= "HS AAU Team Name Deleted Successfully!";
		$count=$func->totalRows(TBL_HS_AAU_TEAM_NAME);
...
if($_REQUEST['mode']=="delAll"){
	...
	for($i=0;$i<count($_POST['check_delete']);$i++)
	{
	 ...
	 $delete_query_details = "delete from ".TBL_HS_AAU_TEAM_NAME." where fldId=".$_POST['check_delete'][$i];
		$delmsg = $db->query($delete_query_details);
	}
	if(isset($delmsg)){
...
function deleteRecord(fldId,page) {
	...
	if( confirm("Are you sure to delete the HS AAU Team Name?")) {
...
function submitDeleteAll(){
	...
	if(countChecked!=0){
		if( confirm("Are you sure to delete the selected HS AAU Team Name?")) {
		...
		alert("Please select at least one HS AAU Team Name to delete");
		return false;
	}
			...
			<TD bgColor=#ffffff>
				...
				<!--Main Center Content BEGIN -->
						...
						<table border="1" cellpadding="1" cellspacing="0" width="100%" bordercolor="#808080" style="border-collapse:collapse" class="tablePadd"> 
							...
							<strong>HS AAU Team Name Listing</strong> </td></tr>
				...
				<input type="hidden" name="page" id="page" value="0">
						...
						<?
						if(!$searchname){
						 $query =" Select * from ".TBL_HS_AAU_TEAM_NAME." where fldStatus=1"; }else{
