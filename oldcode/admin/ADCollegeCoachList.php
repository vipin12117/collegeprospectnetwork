...
if($_REQUEST['mode']=="del")
{
	...
	$delete_query_details = "delete from ".TBL_COLLEGE_COACH_REGISTER." where fldId='".$fldId."'";
	$delmsg = $db->query($delete_query_details);
	if(isset($delmsg)){
		$_REQUEST['msg']= "College Deleted Successfully!";
		$count=$func->totalRows(TBL_COLLEGE_COACH_REGISTER);
...
?>
<HTML><HEAD><TITLE>College Coach Listing</TITLE>
...
function submitDeleteAll(){
	...
	if(countChecked!=0){
		if( confirm("Are you sure to delete the selected College?")) {
		...
		alert("Please select at least one College to delete");
		return false;
	}
			...
			<TD bgColor=#ffffff>
				...
				<!--Main Center Content BEGIN -->
						...
						<table border="1" cellpadding="1" cellspacing="0" width="100%" bordercolor="#808080" style="border-collapse:collapse" class="tablePadd"> 
							...
							<strong>College Coach Listing</strong> </td></tr>
				...
				<input type="hidden" name="page" id="page" value="0">
							...
							<input type="submit"  name="search" id="search" style="width:180px" value="Search College By Name">
							</form></td></tr>
							<form name="frmUsers" action="" method="post" onsubmit="">
						<?
  if(!$searchname){
$query =" Select fldId,fldUserName,fldCollegecode, fldCollegename, fldCity, fldState,fldNeedType,fldCoachname from ".TBL_COLLEGE_COACH_REGISTER; }else{
$query ="Select fldId,fldUserName,fldCollegecode, fldCollegename, fldCity, fldState,fldNeedType,fldCoachname from ".TBL_COLLEGE_COACH_REGISTER." where 1=1 ".$srchCond;
						}
...
<td align="left" class="normalblack_12" width="20%">&nbsp;<strong>User Name</strong></td>
<td align="left" class="normalblack_12" width="25%">&nbsp;<strong>College Name</strong></td>
...
<td align="left" class="normalblack_12" width="25%">&nbsp;<strong>Sports</strong></td>
';
...
echo '</tr>';
			...
			$pageURL                  = "ADCollegeCoachDetails.php?fldId=$fldId";
...
<td align="left" class="normalblack_12" >'.wordwrap($fldUserName,15,"\n",true).'</td>
<td align="left" class="normalblack_12" >'.wordwrap($fldCollegename,17,"\n",true).'</td>
<td align="left" class="normalblack_12">'.wordwrap($fldCity,10,"\n",true).'</td>
<td align="left" class="normalblack_12">'.wordwrap($fldState,10,"\n",true).'</td>
<td align="left" class="normalblack_12" >'.wordwrap($SportsName,17,"\n",true).'</td>
';

echo '<td class="normalblack_12" align="center"><a href="ADCollegeCoachEdit.php?mode=edit&fldId='.$db->f('fldId').'&page='.$pageno.'"><img src="images/b_edit.png" border="0" title="Edit"></a></td>';
