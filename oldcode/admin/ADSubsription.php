...
if($_POST['isSubmit']=='save'){
		...
		$fldDescription = $func->input_fun($_POST['fldCost']);
...
$fldPeriod 		= $func->input_fun($_POST['fldPeriod']);
		...
		if($db->MatchingRec(TBL_SUBSRIPTION,$whereClause)>0) {    #user Username already exists 
			$error_msg = 'This Subsription Already Exists!';
			$flag++;
			}
        ...
        if($flag==0){
			...
			'fldPeriod' 			=> $func->input_fun($_POST['fldPeriod'])
	 		...
	 		$db->insertRec(TBL_SUBSRIPTION,$strDataArr);
       
			#redirect to listing page on successfull updation
			
			header("Location: ADSubsriptionList.php?msg=Subsription is Added Successfully, ");
		}
	...
	if($error_msg!=""){
		...
		$fldDescription   = $_REQUEST['fldCost'];
...
<!-- MAin Content Starts From Here -->
			...
			<option value="Quaterly" <?if($fldStatus=='Quaterly'){ echo "selected"; }?>>Quaterly</option>
