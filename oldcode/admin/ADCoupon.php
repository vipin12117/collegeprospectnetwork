...
if($_POST['isSubmit']=='save'){
        ...
        //Edit the Coupon
		$coupon_name   	     = $func->input_fun($_POST['cpn_number']);
		$discount_amount     = $func->input_fun($_POST['discount_amount']);
		$StartDate        = $func->input_fun($_POST['StartDate']);
		$EndDate 		 = $func->input_fun($_POST['EndDate']);
        ...
        if($flag==0){
			...
			//Insert Coupon
	 		...
	 		$db->insertRec($tbl_coupon,$strDataArr);
       
			#redirect to listing page on successfull updation
			
			header("Location: ADCouponManage.php?msg=Coupon is Added Successfully ");
		}
	if($error_msg!=""){
