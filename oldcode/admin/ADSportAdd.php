...
if($_POST['isSubmit']=='save'){
		...
		$whereClause = "fldSportsname='".$fldSportsname."'";

		if($db->MatchingRec(TBL_SPORTS,$whereClause)>0) {    #user Username already exists
			$error_msg = 'This Coach is Already Exists!';
			$flag++;
			}
        ...
        if($flag==0){
	 	...
	 	$db->insertRec(TBL_SPORTS,$strDataArr);
			#redirect to listing page on successfull updation
			
			header("Location: ADSportList.php?msg=$fldSportsname is Added Successfully, ");
		}
...
function validate(){
	...
	if(trimString(document.frmUsers.fldSportsname.value) == ""){
		error_msg += "Please Enter Sports Name! \n";
	}
	else{
		if(hasSpecialCharaters(document.frmUsers.fldSportsname.value)){
			error_msg += "Enter Sports Name! \n";
		}
	}
...
<HTML><HEAD><TITLE>Add Sports Info</TITLE>
...
<!-- MAin Content Starts From Here -->
		...
		<tr height="20">
			<td valign="top" align="right" class="normalblack_12" width="30%">Sports Name<font color="red">*</font> </td>
...
<td valign="top"  align="center" class="normalblack_12" > : </td><td valign="top" align="left" class="normalblack_12">
<textarea name="fldDescription" id="fldDescription" value="<?=$fldDescription?>" rows="5" cols="15"  style="width:220px"
onKeyDown="textCounter(document.frmUsers.fldDescription,document.frmUsers.remLen2,250)"
onKeyUp="textCounter(document.frmUsers.fldDescription,document.frmUsers.remLen2,250)"><?=$fldDescription?></textarea>
