...
?>
<HTML><HEAD><TITLE>Sports Listing</TITLE>
...
function deleteRecord(fldId,page) {
	...
	if( confirm("Are you sure to delete the Sport?")) {
...
function submitDeleteAll(){
	...
	if(countChecked!=0){
		if( confirm("Are you sure to delete the selected sport?")) {
		...
		alert("Please select at least one sport to delete");
		return false;
	}
...
<TD class="heading">
 <TABLE cellSpacing=0 cellPadding=1 width="95%" align=center border=0>
			...
			<TD bgColor=#ffffff>
				...
				<!--Main Center Content BEGIN -->
						...
						<table border="1" cellpadding="1" cellspacing="0" width="100%" bordercolor="#808080" style="border-collapse:collapse" class="tablePadd"> 
							...
							<strong>Sport Listing</strong> </td></tr>
	...
	<input type="submit"  name="search" id="search" style="width:180px" value="Search Sport By Name">
							</form></td></tr>
						...
						<?
  if(!$searchname){
$query =" Select fldId,  fldSportsname from ".TBL_SPORTS; }else{
$query ="Select  fldId,  fldSportsname from ".TBL_SPORTS." where 1=1 ".$srchCond;
						}
...
<td align="left" class="normalblack_12" width="25%">&nbsp;<strong>Sports Name</strong></td>';
...
echo '</tr>';
			...
			$pageURL            = "ADSportDetails.php?fldId=$fldId";//adduserdeatil
			$detailsWindowTitle = "AdminSportDetails";
...
<td align="left" class="normalblack_12" >'.wordwrap($fldSportsname,17,"\n",true).'</td>';


echo '<td class="normalblack_12" align="center">
<a href="ADSportEdit.php?mode=edit&fldId='.$db->f('fldId').'&page='.$pageno.'"><img src="images/b_edit.png" border="0" title="Edit"></a></td>';
