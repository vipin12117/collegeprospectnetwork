...
if($_POST['isSubmit']=='save'){
		...
		$fldContactInfo = $func->input_fun($_POST['fldContactInfo']);
		$fldStatus      = $func->input_fun($_POST['fldStatus']);

		$whereClause = "fldName='".$func->input_fun($_POST['fldName'])."'";

		if($db->MatchingRec(TBL_HS_AAU_TEAM_NAME,$whereClause)>0) {    #user Username already exists
			$error_msg = 'This HS/AAU Team Name Already Exists!';
			$flag++;
			}
        ...
        if($flag==0){
			...
			'fldContactInfo' 		=> $func->input_fun($_POST['fldContactInfo']),
	 		...
	 		$db->insertRec(TBL_HS_AAU_TEAM_NAME,$strDataArr);
       
			#redirect to listing page on successfull updation
			
			header("Location: HsAauTeamList.php?msg=HS/AAU Team Name is added Successfully, ");
		}
	...
	if($error_msg!=""){
		...
		$fldContactInfo   = $_REQUEST['fldContactInfo'];
...
<HTML><HEAD><TITLE>Add HS / AAU Team Name Info</TITLE>
...
<!-- MAin Content Starts From Here -->
			...
			<tr height="20">
			<td valign="top" align="right" class="normalblack_12" width="30%">Contact<font color="red"></font> </td>
			<td valign="top"  align="center" class="normalblack_12" > : </td>
			<td valign="top" align="left" class="normalblack_12"><textarea name="fldContactInfo" id="fldContactInfo" rows="10" cols="15"  style="width: 220px;" onKeyDown="textCounter(document.frmPage.fldPageDescraption,document.frmPage.remLen2,100000)"
             onKeyUp="textCounter(document.frmPage.fldPageDescraption,document.frmPage.remLen2,100000)" class="txt"><?php echo $fldAddress; ?></textarea></td>
			</tr>
			<tr height="20">
			<td align="center" valign="top" colspan="3" class="normalwhite_14">
