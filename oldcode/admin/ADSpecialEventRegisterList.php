...
if($_REQUEST['mode']=="del")
{
	...
	$delete_query_details = "delete from ".TBL_SPECIAL_EVENT_REGISTER." where fldId=".$fldId;
	$delmsg = $db->query($delete_query_details);
	if(isset($delmsg)){
		$_REQUEST['msg']= "Special Event Register Deleted Successfully!";
		$count=$func->totalRows(TBL_SPECIAL_EVENT_REGISTER);
...
if($_REQUEST['mode']=="delAll"){
	...
	for($i=0;$i<count($_POST['check_delete']);$i++)
	{
	 ...
	 $delete_query_details = "delete from ".TBL_SPECIAL_EVENT_REGISTER." where fldId=".$_POST['check_delete'][$i];
		$delmsg = $db->query($delete_query_details);
	}
	if(isset($delmsg)){
			...
			<TD bgColor=#ffffff>
				...
				<!--Main Center Content BEGIN -->
						...
						<table border="1" cellpadding="1" cellspacing="0" width="100%" bordercolor="#808080" style="border-collapse:collapse" class="tablePadd"> 
							...
							<strong>Special Event Registration Listing</strong> </td></tr>
				...
				<input type="hidden" name="page" id="page" value="0">
							...
							<input type="submit"  name="search" id="search" style="width:180px" value="Search Special Event by name">
							</form></td></tr>
							<form name="frmCatagory" action="" method="post" onSubmit="">
						<?
						if(!$searchname){
						$query =" Select * from ".TBL_SPECIAL_EVENT_REGISTER." where fldpaymentstatus='PAID'"; }else{
						$query ="Select * from ".TBL_SPECIAL_EVENT_REGISTER." where fldpaymentstatus='PAID' AND 1=1 ".$srchCond;
						}
						...
						echo '<tr><td align="center" class="normalblack_12" width="4%">&nbsp;</td>
									...
									<td align="left" class="normalblack_12" width="25%">&nbsp;<strong>Registered Name</strong></td>
                                   ';
						...
						for ($i=0;$i<$db->num_rows();$i++) {
							...
							$fldName = $fldFirstName." ".$fldLastName;
														
							$pageURL            = "ADSpecialEventRegisterDetails.php?fldId=$fldId";
							$detailsWindowTitle = "AdminSpecialEventRegisterDetails";
							...
							echo '<tr><td align="center" class="normalblack_12">&nbsp;';
									...
									';
							   		...
							   		//echo '<td class="normalblack_12" align="center"><a href="ADSpecialEventEdit.php?mode=edit&fldId='.$db->f('fldId').'&page='.$pageno.'"><img src="images/b_edit.png" border="0" title="Edit"></a></td>';
