...
class ImageComponent extends Component {
	...
	public function set_zoom_crop($zoom_crop) {
		...
		if($zoom_crop === 1 OR $zoom_crop === 'C'){
			$this->zoom_crop = 1;
		}
		elseif(extension_loaded('magickwand') AND in_array($zoom_crop, $allowed_zoom_crop_param)){
	...
	public function upload_image($field) {
		...
		AND is_array($this->controller->request->data[$model][$value])) {

			// get pointer for lighter code
			$file = &$this->controller->request->data[$model][$value];
